...
def run(_run, _config, _log):
    ...
    unique_token = f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_seed{_config['seed']}"

    args.unique_token = unique_token
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(
            dirname(dirname(dirname(abspath(__file__)))), args.local_results_path, "tb_logs", args.env, map_name, args.name
        )
...
def evaluate_sequential(args, runner, test_episodes=10):
    ...
    save_dir = os.path.join(args.local_results_path, 'renders', args.env, args.env_args['map_name'], args.name, f'{args.unique_token}')
...
def run_sequential(args, logger):
    ...
    while runner.t_env <= args.t_max:
        ...
        # Do Not save model while training
        ...
        #         args.local_results_path, "models", args.env, args.env_args['map_name'], args.name, args.unique_token, str(runner.t_env)
        #     )
    ...
    # save buffer and final model
    save_path_buffer = os.path.join(
        args.local_results_path, 'replaybuffer', args.env, args.env_args['map_name'], args.name, args.unique_token
    )
    os.makedirs(save_path_buffer, exist_ok=True)
    save_path_buffer = os.path.join(save_path_buffer, 'buffer.pkl')
